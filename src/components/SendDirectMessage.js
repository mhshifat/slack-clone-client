...
import { createDirectMessageMutation } from "../graphql/mutations";
...
class SendDirectMessage extends Component {
  ...
  handleFormSubmit = catchAsync(async e => {
    ...
    const {
      data: {
        createDirectMessage: { success, errors }
      }
    } = await mutate({
      variables: { ...form, receiverId: userId }
    });
...
export default graphql(createDirectMessageMutation)(SendDirectMessage);
